...
public:
    Expectation_maximization(int number_of_centers, const std::vector<double>& data, 
        const std::vector<int>& initial_centers = std::vector<int>(), int maximum_iteration = 100)
        : points(data.begin(), data.end()), threshold(1e-3), maximum_iteration(maximum_iteration), is_converged(false), seed(time(NULL))
    {
        srand(seed);
