...
public:
    Expectation_maximization(int number_of_centers, const std::vector<double>& data, 
        const std::vector<int>& initial_centers = std::vector<int>(), int number_of_runs = 50, int maximum_iteration = 100)
        : points(data.begin(), data.end()), threshold(1e-4), maximum_iteration(maximum_iteration), is_converged(false), seed(time(NULL))
    {
        srand(seed);
...
protected:
    ...
    void initiate_centers_from_memberships(int number_of_centers, const std::vector<int>& initial_centers)
    {
        centers.clear();
        /* Calculate mean */
    ...
    void calculate_fitting_with_multiple_run(int number_of_centers, int number_of_run)
    {
