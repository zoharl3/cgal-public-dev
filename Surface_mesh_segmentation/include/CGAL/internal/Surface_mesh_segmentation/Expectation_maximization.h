...
private:
    ...
    public:
        ...
        { }
        Gaussian_center(double mean, double deviation = 0.0, double mixing_coefficient = 0.0)
        ...
        {
            if(deviation == 0.0) { return x == mean ? 1.0 : 0.0; }
...
public:   
    ...
    {
        CGAL_assertion(data.size() >= number_of_centers && "Number of centers can not be more than number of data.");

        // For initialization with k-means, with one run
        if(init_type == K_MEANS_INITIALIZATION)
        {
            K_means_clustering k_means(number_of_centers, data, K_means_clustering::PLUS_INITIALIZATION,
...
private:
    ...
    {   
        ...
        for(std::vector<Gaussian_center>::iterator it = centers.begin(); it != centers.end(); ++it) {
