...
public:
    ...
    Expectation_maximization(int number_of_centers, const std::vector<double>& data, 
       double threshold = DEF_THRESHOLD,  int maximum_iteration = DEF_MAX_ITER)
        : points(data), threshold(threshold), maximum_iteration(maximum_iteration), is_converged(false), seed(0),
    ...
    Expectation_maximization(int number_of_centers, const std::vector<double>& data, std::vector<int>& initial_center_ids,
        double threshold = DEF_THRESHOLD, int maximum_iteration = DEF_MAX_ITER)
        : points(data), threshold(threshold), maximum_iteration(maximum_iteration), is_converged(false), seed(0),
        final_likelihood(-(std::numeric_limits<double>::max)())
    {
        number_of_centers = initiate_centers_from_memberships(number_of_centers, initial_center_ids);
        membership_matrix = std::vector<std::vector<double> >(number_of_centers, std::vector<double>(points.size()));
        fit();
        //write_center_parameters("centers_param.txt");
    }
    /* For initialization with random center selection (Forgy), with multiple run */ 
    Expectation_maximization(int number_of_centers, const std::vector<double>& data, int number_of_runs, 
        double threshold = DEF_THRESHOLD,  int maximum_iteration = DEF_MAX_ITER)
        : points(data), threshold(threshold), maximum_iteration(maximum_iteration), is_converged(false), 
    ...
    // Experimental! 
    Expectation_maximization(int number_of_centers, const std::vector<double>& data, int number_of_runs,
        std::vector<std::vector<double> >& probabilities, std::vector<int>& data_centers,
        double threshold = DEF_THRESHOLD,  int maximum_iteration = DEF_MAX_ITER)
        : points(data), threshold(threshold), maximum_iteration(maximum_iteration), is_converged(false), 
          seed(static_cast<unsigned int>(time(NULL))),
          membership_matrix(std::vector<std::vector<double> >(number_of_centers, std::vector<double>(points.size()))),
          final_likelihood(-(std::numeric_limits<double>::max)())
    {
        ...
        srand(DEF_SEED);
        #endif
        average_with_multiple_run(number_of_centers, number_of_runs, probabilities, data_centers);    
    }
    ...
    void fill_with_minus_log_probabilities(std::vector<std::vector<double> >& probabilities)
    {
        double epsilon = 1e-5; 
        ...
        for(std::size_t point_i = 0; point_i < points.size(); ++point_i)
        {
            ...
            for(std::size_t center_i = 0; center_i < centers.size(); ++center_i)
            {
                ...
                probabilities[center_i][point_i] = (std::max)(probability, std::numeric_limits<double>::epsilon());
            }
        }                       
    } 
    ...
    void refresh_parameters_and_probabilities(int number_of_centers,
        std::vector<int>& initial_center_ids, std::vector<std::vector<double> >& probabilities)
    {
        std::vector<bool> cluster_exist(number_of_centers, false);
        std::vector<int>  cluster_shift(number_of_centers, 0);
        for(std::vector<int>::iterator id_it = initial_center_ids.begin(); id_it != initial_center_ids.end(); ++id_it)
        {
        ...
        double epsilon = 1e-5; // this epsilon should be consistent with epsilon in calculate_dihedral_angle_of_edge!
        probabilities = std::vector<std::vector<double> >
        ...
        for(std::size_t point_i = 0; point_i < points.size(); ++point_i)
        {
            ...
            for(std::size_t center_i = 0; center_i < centers.size(); ++center_i)
            {
                ...
                probability = (std::max)(probability, epsilon);
                probability = -log(probability);
                probabilities[center_i][point_i] = (std::max)(probability, 1e-5); // this is another epsilon, edge can not hold 0 to source in graph-cut.
            }
        }  
...
protected:
    ...
    int initiate_centers_from_memberships(int number_of_centers, std::vector<int>& initial_center_ids)
    {
    ...
    double calculate_likelihood_with_log_sum_exp()
    {
         ...
         */
        double likelihood = 0.0;
        std::vector<double> max_probability(points.size(), -(std::numeric_limits<double>::max)());
        for(std::size_t point_i = 0; point_i < points.size(); ++point_i)
        {           
            for(std::size_t center_i = 0; center_i < centers.size(); ++center_i)
            {
                double membership = centers[center_i].log_probability_with_coef(points[point_i]);
                if(max_probability[point_i] < membership) { max_probability[point_i] = membership; }
                membership_matrix[center_i][point_i] = membership;
            }
            double total_membership = 0.0;
            for(std::size_t center_i = 0; center_i < centers.size(); ++center_i)
            {
                ...
                double membership = exp(membership_matrix[center_i][point_i] - max_probability[point_i]);
                total_membership += membership;
                membership_matrix[center_i][point_i] = membership;
            }
            for(std::size_t center_i = 0; center_i < centers.size(); ++center_i)
    ...
    // Experimental! 
    void average_with_multiple_run(int number_of_centers, int number_of_run, std::vector<std::vector<double> >& probabilities, std::vector<int>& data_centers)
    {
