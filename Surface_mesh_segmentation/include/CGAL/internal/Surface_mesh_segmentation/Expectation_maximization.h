...
public:
    ...
    Expectation_maximization(int number_of_centers, const std::vector<double>& data, 
       double threshold = DEF_THRESHOLD,  int maximum_iteration = DEF_MAX_ITER, bool use_matrix = USE_MATRIX)
        : points(data.begin(), data.end()), threshold(threshold), maximum_iteration(maximum_iteration), is_converged(false), seed(0),
          probability_matrix( use_matrix ? std::vector<std::vector<double> >(number_of_centers, std::vector<double>(points.size())) :
    ...
    Expectation_maximization(int number_of_centers, const std::vector<double>& data, const std::vector<int>& initial_center_ids,
        double threshold = DEF_THRESHOLD, int maximum_iteration = DEF_MAX_ITER, bool use_matrix = USE_MATRIX)
        : points(data.begin(), data.end()), threshold(threshold), maximum_iteration(maximum_iteration), is_converged(false), seed(0),
          probability_matrix( use_matrix ? std::vector<std::vector<double> >(number_of_centers, std::vector<double>(points.size())) :
            std::vector<std::vector<double> >()) 
    ...
    Expectation_maximization(int number_of_centers, const std::vector<double>& data, int number_of_runs, 
        double threshold = DEF_THRESHOLD,  int maximum_iteration = DEF_MAX_ITER, bool use_matrix = USE_MATRIX)
        : points(data.begin(), data.end()), threshold(threshold), maximum_iteration(maximum_iteration), is_converged(false), 
          seed(static_cast<unsigned int>(time(NULL))),
          probability_matrix( use_matrix ? std::vector<std::vector<double> >(number_of_centers, std::vector<double>(points.size())) :
            std::vector<std::vector<double> >())
    {
        srand(seed);
        calculate_fitting_with_multiple_run(number_of_centers, number_of_runs, use_matrix);    
    }
...
protected:
    ...
    double calculate_likelihood_with_matrix()
    {
        ...
        for(std::size_t point_i = 0; point_i < points.size(); ++point_i)
        {
            ...
            for(std::size_t center_i = 0; center_i < centers.size(); ++center_i)
            {
                double membership = centers[center_i].probability_with_coef(points[point_i].data);
                total_membership += membership;
    ...
    {
        // E-step
        if(first_iteration) { calculate_likelihood_with_matrix(); }
        // M-step 
    ...
    void calculate_fitting_with_multiple_run(int number_of_centers, int number_of_run, bool use_matrix)
    {
        ...
        while(number_of_run-- > 0)
        {
            ...
            double likelihood = use_matrix ? calculate_fitting_with_matrix() : calculate_fitting();
