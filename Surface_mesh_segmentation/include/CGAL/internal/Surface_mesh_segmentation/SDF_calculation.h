...
protected:
    ...
    typedef typename AABB_const_polyhedron_triangle_primitive<Kernel, Polyhedron> Primitive;
...
public:   
    ...
    : cone_angle(cone_angle), number_of_rays(number_of_rays), use_minimum_segment(false), multiplier_for_segment(1)
    { }
...
protected:
    double calculate_sdf_value_of_facet(Facet_const_iterator& facet, const Tree& tree, const Disk_samples_list& samples) const
    {
    ...
    boost::tuple<double, double> remove_outliers_and_calculate_sdf_value( std::vector<double>& ray_distances,
    ...
    {    
             ...
             ++dist_it, ++w_it)
        {
            ...
            if(abs((*dist_it) - median_sdf) > (st_dev * CGAL_ST_DEV_MULTIPLIER)) { continue; }
            total_distance += (*dist_it) * (*w_it);
            total_weights += (*w_it);
            not_outlier_count++;
        }
