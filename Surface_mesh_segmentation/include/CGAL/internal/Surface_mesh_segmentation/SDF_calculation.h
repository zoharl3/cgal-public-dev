...
private:
         ...
         const Disk_samples_list& samples) const
    {
      ...
      #define SHOOT_ONLY_RAYS
        ...
        boost::tie(sdf_result, acceptance_rate) = remove_outliers_and_calculate_sdf_value(ray_distances);
    ...
    remove_outliers_and_calculate_sdf_value(std::vector<std::pair<double, double> >& ray_distances) const
    {  
        ...
        for(Pairs_vector::iterator it = ray_distances.begin(); it != ray_distances.end(); ++it)
        {
        ...
        for(Pairs_vector::iterator it = ray_distances.begin(); it != ray_distances.end(); ++it)
        {     
            if(std::abs(it->first - median_sdf) > (st_dev * CGAL_ST_DEV_MULTIPLIER)) { continue; }
            total_distance += it->first * it->second;
            total_weights += it->second;
            not_outlier_count++;
        }
