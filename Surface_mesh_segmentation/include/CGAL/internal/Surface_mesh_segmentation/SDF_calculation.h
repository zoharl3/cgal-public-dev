...
private:
    ...
    typedef Vogel_disk_sampling<boost::tuple<double, double, double> > Default_sampler;
// member variables
private:  
    GeomTraits traits;
...
public:   
    ...
    SDF_calculation(InputIterator polyhedron_begin, InputIterator polyhedron_end, bool build_kd_tree = false, GeomTraits traits = GeomTraits())
        : traits(traits),
        angle_functor(traits.angle_3_object()),
        scale_functor(traits.construct_scaled_vector_3_object()),
        sum_functor(traits.construct_sum_of_vectors_3_object()),
        ...
        centroid_functor(traits.construct_centroid_3_object())
    {
      ...
      {
        tree.insert((*polyhedron_begin)->facets_begin(), (*polyhedron_begin)->facets_end());
      }
      tree.build();
    ...
    */
    ...
    {        
        ...
        return calculate_sdf_value_of_point(center, normal, skip, visitor, cone_angle, accept_if_acute, disk_samples);
    }
    ...
    {        
        ...
        v1 = scale_functor(v1, static_cast<typename GeomTraits::FT>(1.0 / CGAL::sqrt(v1.squared_length())));
        v2 = scale_functor(v2, static_cast<typename GeomTraits::FT>(1.0 / CGAL::sqrt(v2.squared_length())));        
           
        std::vector<std::pair<double, double> > ray_distances;
        ray_distances.reserve(disk_samples.size());
        
        const typename GeomTraits::FT length_of_normal = static_cast<typename GeomTraits::FT>( 1.0 / tan(cone_angle / 2.0) );
        ...
        return boost::optional<double>(remove_outliers_and_calculate_sdf_value(ray_distances)); 
    }
...
private:
                ...
                const Disk_samples_list& disk_samples) const
    {
        ...
        CGAL::internal::SkipPrimitiveFunctor<Polyhedron::Facet_const_handle> skip(facet);
        CGAL::internal::FirstIntersectionVisitor<Polyhedron::Facet_const_handle> visitor;
