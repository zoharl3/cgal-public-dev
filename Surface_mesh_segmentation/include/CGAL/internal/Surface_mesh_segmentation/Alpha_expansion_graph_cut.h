...
public:
    ...
    Alpha_expansion_graph_cut(std::vector<std::pair<int, int> >& edges,
        std::vector<double>& edge_weights, std::vector<int>& labels, std::vector<std::vector<double> >& probability_matrix, std::vector<int>& center_ids)
    {
        ...
        for(std::size_t vertex_i = 0; vertex_i <  probability_matrix[0].size(); ++vertex_i)
        {
            ...
            tie(source_e, source_e_added) = boost::add_edge(cluster_source, new_vertex, graph);
            tie(source_e_rev, source_e_rev_added) = boost::add_edge(new_vertex, cluster_source, graph);
            
            tie(sink_e, sink_e_added) = boost::add_edge(new_vertex, cluster_sink, graph);
            tie(sink_e_rev, sink_e_rev_added) = boost::add_edge(cluster_sink, new_vertex, graph);
            
            CGAL_assertion(source_e_added && source_e_rev_added &&  sink_e_added && sink_e_rev_added);
            
            //put vertex capacities (to edges between itself and source & sink)
            boost::put(boost::edge_capacity, graph, source_e, probability_matrix[0][vertex_i]);
            boost::put(boost::edge_capacity, graph, source_e_rev, 0);
            boost::put(boost::edge_capacity, graph, sink_e, probability_matrix[1][vertex_i]);
        ...
        for(std::vector<std::pair<int, int> >::iterator edge_it = edges.begin(); edge_it != edges.end();
            ++edge_it, ++weight_it)
        {
        ...
        for(std::vector<Vertex_descriptor>::iterator vertex_it = inserted_vertices.begin(); 
            vertex_it != inserted_vertices.end(); ++vertex_it)
        {
            boost::default_color_type color = boost::get(boost::vertex_color, graph, *vertex_it);
