                  ...
                  std::vector<std::size_t>& labels) const
{
    ...
    do {
            ...
            it != probability_matrix.end(); ++it, ++alpha)
        {
            ...
            if(min_cut - flow < flow * tolerance) { continue; }            
            min_cut = flow;
            success = true;
            //update labeling
    ...
    CGAL_TRACE_STREAM << "vertex creation time: " << vertex_creation_time << std::endl;
...
private:
  // CSR only accepts bundled props
    struct VertexP {
      ...
      // this should be fine...
      boost::compressed_sparse_row_graph<boost::directedS>::edge_descriptor vertex_predecessor;
    };

    struct EdgeP {
      double edge_capacity;
      double edge_residual_capacity;
      boost::compressed_sparse_row_graph<boost::directedS>::edge_descriptor edge_reverse;
    };
                  ...
                  std::vector<std::size_t>& labels) const
{
    ...
    // for debug time
    double vertex_creation_time, edge_creation_time, graph_creation_time, reverse_mapping_time, cut_time;
    vertex_creation_time = edge_creation_time = graph_creation_time = reverse_mapping_time = cut_time = 0.0;
    ...
    do {
            ...
            it != probability_matrix.end(); ++it, ++alpha)
        {
                ...
                ++edge_it, ++weight_it)
            {
                ...
                {     
                    if(label_1 != alpha) 
                    { add_edge_and_reverse(v1, v2, *weight_it, *weight_it, edge_map, edge_map_weights); }   
                }
                else
                {
            ...
            for(boost::tie(ei, ee) = boost::edges(graph); ei != ee; ++ei) {
              ...
              graph[opp_edge.first].edge_reverse = *ei; // and edge_reverse of *ei will be (or already have been) set by the opp_edge 
            }
            reverse_mapping_time += timer.time(); timer.reset();
            ...
            double flow = boost::boykov_kolmogorov_max_flow(graph, 
              ...
              boost::get(boost::vertex_index, graph), // this is not bundled, get it from graph (CRS provides one)
              cluster_source, 
              cluster_sink
              );
        #else
    ...
    CGAL_TRACE_STREAM << "vertex creation time: " << vertex_creation_time << std::endl;
    CGAL_TRACE_STREAM << "edge creation time: " << edge_creation_time << std::endl;
    CGAL_TRACE_STREAM << "graph creation time: " << graph_creation_time << std::endl;
    CGAL_TRACE_STREAM << "reverse mapping time: " << reverse_mapping_time << std::endl;
    CGAL_TRACE_STREAM << "max flow algorithm time: " << cut_time << std::endl;
    return min_cut;
}
};
