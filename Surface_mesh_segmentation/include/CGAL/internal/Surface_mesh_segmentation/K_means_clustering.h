...
public:
    ...
    void forgy_initialization(int number_of_centers, const std::vector<T1>& points, std::vector<T2>& centers, CGAL::Random& random)
    {
        ...
        {    
            std::size_t random_range = points.size() - number_of_centers + i; // activate one more element in each iteration for as selectable
            std::size_t random_index = random.get_int(0, static_cast<int>(random_range) + 1); // [0, random_range];

            std::pair<std::set<std::size_t>::iterator, bool> random_index_unique = selected.insert(random_index);
            if(!random_index_unique.second) {                        
                random_index_unique = selected.insert(random_range); // random_range can not be inside random_index_unique, 
                CGAL_assertion(random_index_unique.second);          // since we just increment it by one 
            }
    ...
    */
     ...
     */
    template<class T1, class T2>
    void plus_plus_initialization(int number_of_centers, const std::vector<T1>& points, std::vector<T2>& centers, CGAL::Random& random)
    {
        if(number_of_centers <= 0) { return; }
        ...
        std::size_t initial_index = random.get_int(0, static_cast<int>(points.size())); // [0, points size)
        ...
        for(int i = 1; i < number_of_centers; ++i)
        {
            ...
            double random_ds = random.get_double(0.0, total_probability); // [0.0, total_probability)
            ...
            std::size_t selection_index = std::upper_bound(distance_square_cumulative.begin(), distance_square_cumulative.end(), random_ds)
...
public: 
    ...
    K_means_center(double mean): mean(mean), new_mean(0.0), new_number_of_points(0), empty(true)
    { }

    K_means_center(const K_means_point& point) : mean(point.data), new_mean(0.0), new_number_of_points(0), empty(true)
    { }
    ...
    void calculate_mean()
    {
        if(new_number_of_points == 0) { // just return and this will leave `mean` as it is,
...
*/
inline bool K_means_point::calculate_new_center(std::vector<K_means_center>& centers)
{
    int new_center_id = 0;
...
public:
    ...
    {  
        CGAL_precondition(number_of_centers > 0 && "Number of centers should be positive.");
        CGAL_precondition(data.size() >= static_cast<std::size_t>(number_of_centers) && "Number of centers can not be more than number of data.");
