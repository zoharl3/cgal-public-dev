...
public:
    ...
    */
     ...
     */
    template<class T1, class T2>
    void plus_plus_initialization(int number_of_centers, const std::vector<T1>& points, std::vector<T2>& centers)
    {
        std::vector<std::size_t> indices(points.size()); // it is required to not to swap points
        for(std::size_t i = 0; i < points.size(); ++i) { indices[i] = i; }
        ...
        for(int i = 1; i < number_of_centers; ++i)
        {
            ...
            std::vector<double>::iterator last_active = distance_square_cumulative.begin() + (points.size() -i -1);
            double zero_one = rand() / (RAND_MAX + 1.0); // [0,1) random number
            
            double random_ds =  zero_one * (*last_active);
            int selection_index = std::upper_bound(distance_square_cumulative.begin(), last_active + 1, random_ds) 
            ...
            std::swap(distance_square[selection_index], distance_square[points.size() -i -1]);            
