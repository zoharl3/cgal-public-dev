...
public:
  ...
  void intersection(const Query& query, const Primitive& primitive)
  {
    ...
    if (! GeomTraits().do_intersect_3_object()(query,primitive.datum()) ) { return; }
    Result m_result = AABBTraits().intersection_object()(query, primitive);
    if(m_result)
    {
        const Point* i_point;  
        double distance;   
        if(!(i_point = CGAL::object_cast<Point>(&m_result->first))) {return;}
...
// just for trying.
    double find_min_distance(const Query& query, const Bounding_box& m_bbox) const
    {
        ...
        double max_bbox[3];
        double origin[3];
        min_bbox[0] = m_bbox.xmin(); max_bbox[0] = m_bbox.xmax();
        min_bbox[1] = m_bbox.ymin(); max_bbox[1] = m_bbox.ymax();
        min_bbox[2] = m_bbox.zmin(); max_bbox[2] = m_bbox.zmax();
