     ...
     int number_of_rays)
{
    ...
    SDF_calculation_class().calculate_sdf_values(mesh, sdf_pmap, cone_angle, number_of_rays);
...
int partition(SDFPropertyMap sdf_pmap, FacetSegmentMap segment_pmap, int number_of_centers,
...
void log_normalize_probability_matrix(std::vector<std::vector<double> >& probabilities) const
{
    ...
    for(std::vector<std::vector<double> >::iterator it_i = probabilities.begin(); it_i != probabilities.end(); ++it_i)
    {
        ...
        {
            double probability = (std::max)(*it, epsilon); // give any facet a little probability to be in any cluster
            probability = -log(probability);
            *it = (std::max)(probability, std::numeric_limits<double>::epsilon()); // zero values are not accepted in max-flow
        }
    }
}
...
{    
    ...
    for(Facet_const_iterator facet_it = mesh.facets_begin(); facet_it != mesh.facets_end(); ++facet_it)
    {
        ...
        {
            std::pair<double, int> sdf_facet_count_pair = depth_first_traversal(facet_it, segment_id, sdf_values, segments);
            double average_sdf_value_for_segment = sdf_facet_count_pair.first / sdf_facet_count_pair.second;
            segments_with_average_sdf_values.push_back(std::pair<int, double>(segment_id, average_sdf_value_for_segment));
            ++segment_id;
        }
    }
...
template<class SegmentPropertyMap, class SDFProperyMap>
std::pair<double, int> 
depth_first_traversal(Facet_const_handle& facet, int segment_id, SDFProperyMap sdf_values, SegmentPropertyMap segments)
{
    ...
    do {
        ...
        {
            const std::pair<double, int>& total_pair = depth_first_traversal(neighbor, segment_id, sdf_values, segments);
