...
public:
    ...
    {    
        ...
        if(!spatial_parameter) { spatial_parameter_actual = window_size / 2.0; }
        else                   { spatial_parameter_actual = *range_parameter; }
        ...
        {               
            ...
            if(!range_parameter)
            {
                ...
                for(typename std::map<Facet_const_handle, std::size_t>::iterator it = neighbors.begin(); it != neighbors.end(); ++it)
                {
            ...
            for(typename std::map<Facet_const_handle, std::size_t>::iterator it = neighbors.begin(); it != neighbors.end(); ++it)
            {
                double spatial_weight = gaussian_function(static_cast<double>(it->second), spatial_parameter_actual);
                double range_weight = gaussian_function(values[it->first] - current_sdf_value, range_parameter_actual);
