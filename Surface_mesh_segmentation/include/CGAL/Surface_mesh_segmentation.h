...
//member functions
public:
Surface_mesh_segmentation(Polyhedron* mesh) : mesh(mesh), log_file("log_file.txt"), number_of_centers(5)
{
    ...
    std::vector<FT>& ray_weights) const
{
    ...
    else
    {
        ...
        if( accepted_ray_count % 2 == 0)
        {
            ...
            FT median_2 = *std::max_element(ray_distances.begin(), ray_distances.begin() + half_ray_count);
