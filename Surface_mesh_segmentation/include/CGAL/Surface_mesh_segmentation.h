...
//member functions
public:
Surface_mesh_segmentation(Polyhedron* mesh) : mesh(mesh), log_file("log_file.txt")
{
...
void calculate_sdf_values()
{
        ...
        facet_it != mesh->facets_end(); ++facet_it)
    {
        ...
        FT sdf = calculate_sdf_value_of_facet(facet_it, center, normal, tree, (1.0/3.0) * PI, 6);
     ...
     const Vector& normal_const, const Tree& tree, double half_cone_angle, int ray_count_sqrt) const
{
    ...
    {
        if(abs((*dist_it) - average_sdf) > st_dev) { continue; }
...
void normalize_sdf_values()
{
    ...
    {   
        ...
        double log_normalized = log(CGAL::to_double(linear_normalized) * NORMALIZATION_ALPHA + 1) / LOG_5; // how to log a generic Number_type(FT)?.
