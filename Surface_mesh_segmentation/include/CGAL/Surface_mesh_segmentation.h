        ...
        sample_it != disk_samples.end(); ++sample_it)
    {
        ...
        else // use segment_distance to limit rays as segments
        {
            ...
            { 
                ...
                boost::tie(is_intersected, intersection_is_acute, min_distance) = cast_and_return_minimum(ray, tree, facet);
                if(!intersection_is_acute) { continue; } 
                min_distance += CGAL::sqrt(segment.squared_length()); // since our source is segment.target()
            }
...
{ 
    ...
    for(int i = 0; i < iteration; ++i)
    {
        ...
        {   
            ...
            {
                double weight =  exp(-0.5 * (pow(it->second / (window_size/2.0), 2))); // window_size => 2*sigma
...
{ 
    ...
    for(int i = 0; i < iteration; ++i)
    {
        ...
        {   
            ...
            for(std::map<Facet_handle, int>::iterator it = neighbors.begin(); it != neighbors.end(); ++it)
            {
                double spatial_weight =  exp(-0.5 * (pow(it->second / (window_size/2.0), 2))); // window_size => 2*sigma                
                double domain_weight  =  exp(-0.5 * (pow((sdf_values[it->first] - current_sdf_value) / (2*deviation), 2)));
