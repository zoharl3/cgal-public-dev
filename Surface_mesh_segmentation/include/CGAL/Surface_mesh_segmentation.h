                            ...
                            int number_of_ray = CGAL_DEFAULT_NUMBER_OF_RAYS)
{
    ...
    SEG_DEBUG(std::cerr <<"Normalization and smoothing time: " << t.time() << std::endl)
}
...
{ 
    ...
    for(int i = 0; i < iteration; ++i)
    {
        ...
        {               
            ...
            {
                double weight = gaussian_function(it->second, window_size/2.0); // window_size => 2*sigma
...
{ 
    ...
    for(int i = 0; i < iteration; ++i)
    {
        ...
        {   
            ...
            {
                median_sdf += *std::max_element(sdf_of_neighbors.begin(), sdf_of_neighbors.begin() + half_neighbor_count);
                median_sdf /= 2;
            }
 ...
 */
void get_neighbors_by_edge(Facet_const_handle& facet, int max_level, std::map<Facet_const_handle, int>& neighbors)
{
 ...
 * @param[out] neighbors visited facets and their distances to root facet
 */
void get_neighbors_by_vertex(Facet_const_handle& facet, std::map<Facet_const_handle, int>& neighbors, int max_level)
{
    typedef std::pair<Facet_const_handle, int> facet_level_pair;
    std::queue<facet_level_pair> facet_queue;
    facet_queue.push(facet_level_pair(facet, 0));
