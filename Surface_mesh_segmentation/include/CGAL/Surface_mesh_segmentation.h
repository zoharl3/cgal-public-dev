...
Surface_mesh_segmentation(Polyhedron* mesh, 
    int number_of_rays_sqrt = 5, double cone_angle = (2.0 / 3.0) * CGAL_PI, int number_of_centers = 5);

void calculate_sdf_values();

double calculate_sdf_value_of_facet (const Facet_handle& facet, const Tree& tree, const Disk_samples_list& samples) const;
...
boost::tuple<double, double> calculate_sdf_value_from_rays(std::vector<double>& ray_distances, std::vector<double>& ray_weights) const;
...
: mesh(mesh), cone_angle(cone_angle), number_of_rays_sqrt(number_of_rays_sqrt), 
    number_of_centers(number_of_centers), log_file("log_file.txt"), use_minimum_segment(false), multiplier_for_segment(1), smoothing_lambda(23.0)
{
...
template <class Polyhedron>
inline double Surface_mesh_segmentation<Polyhedron>::calculate_sdf_value_of_facet(const Facet_handle& facet, const Tree& tree, const Disk_samples_list& samples) const
{
  // AF: Use const Point&
  //IOY: Done, and I am going to change other places (where it is approprite) like this.
    ...
    boost::tie(sdf_result, acceptance_rate) = calculate_sdf_value_from_rays(ray_distances, ray_weights);
    
    if(acceptance_rate > CGAL_ACCEPTANCE_RATE_THRESHOLD || samples.size() == disk_samples_dense.size()) { return sdf_result; }
...
inline boost::tuple<bool, bool, double> Surface_mesh_segmentation<Polyhedron>::cast_and_return_minimum(
...
{   
        ...
        op_it != intersections.end() ; ++op_it) 
    {
        ...
        const Point* i_point; 
        if(!(i_point = CGAL::object_cast<Point>(&object))) { continue; } // Continue in case of segment.
...
template <class Polyhedron>
template <class Query> 
inline boost::tuple<bool, bool, double> Surface_mesh_segmentation<Polyhedron>::cast_and_return_minimum_use_closest (const Query& ray, const Tree& tree, 
...
template <class Polyhedron>
inline boost::tuple<double, double> Surface_mesh_segmentation<Polyhedron>::calculate_sdf_value_from_rays( std::vector<double>& ray_distances,
...
{    
         ...
         ++dist_it, ++w_it)
    {
      // AF: replace fabs with CGAL::abs
        if(CGAL::abs((*dist_it) - median_sdf) > (st_dev * CGAL_ST_DEV_MULTIPLIER)) { continue; }
        total_distance += (*dist_it) * (*w_it);
        total_weights += (*w_it);
        not_outlier_count++;
    }
 ...
 */
template <class Polyhedron>
inline void Surface_mesh_segmentation<Polyhedron>::arrange_center_orientation(const Plane& plane, const Vector& unit_normal, Point& center) const
{
    /*
...
template <class Polyhedron>
inline double Surface_mesh_segmentation<Polyhedron>::calculate_dihedral_angle_of_edge(const Halfedge_handle& edge) const
{
...
template <class Polyhedron>
inline double Surface_mesh_segmentation<Polyhedron>::calculate_dihedral_angle_of_edge_2(const Halfedge_handle& edge) const
{
    double epsilon = 1e-5; // not sure but should not return zero for log(angle)...
...
template <class Polyhedron>
inline void Surface_mesh_segmentation<Polyhedron>::disk_sampling_vogel_method(Disk_samples_list& samples, int ray_count)
{
    double length_of_normal = 1.0 / tan(cone_angle / 2);
...
template <class Polyhedron>
inline void Surface_mesh_segmentation<Polyhedron>::smooth_sdf_values_with_gaussian()
{ 
...
template <class Polyhedron>
inline void Surface_mesh_segmentation<Polyhedron>::smooth_sdf_values_with_median()
{ 
...
template <class Polyhedron>
inline void Surface_mesh_segmentation<Polyhedron>::smooth_sdf_values_with_bilateral()
{ 
...
template <class Polyhedron>
inline void Surface_mesh_segmentation<Polyhedron>::get_neighbors_by_edge(const Facet_handle& facet, std::map<Facet_handle, int>& neighbors, int max_level)
{
...
template <class Polyhedron>
inline void Surface_mesh_segmentation<Polyhedron>::get_neighbors_by_vertex(const Facet_handle& facet, std::map<Facet_handle, int>& neighbors, int max_level)
{
    typedef std::pair<Facet_handle, int> facet_level_pair;
    std::queue<facet_level_pair> facet_queue;
    facet_queue.push(facet_level_pair(facet, 0));
    while(!facet_queue.empty())
    {
        ...
        if(inserted && pair.second < max_level)
        {
            ...
            do {
                ...
                typename Facet::Halfedge_around_vertex_circulator facet_circulator = vertex->vertex_begin();
                do {
...
template <class Polyhedron>
inline void Surface_mesh_segmentation<Polyhedron>::apply_GMM_fitting_and_K_means()
{
    centers.clear();
    std::vector<double> sdf_vector;
    sdf_vector.reserve(sdf_values.size());
    ...
    internal::Expectation_maximization gmm_random_init(number_of_centers, sdf_vector, 50);
    
    internal::K_means_clustering k_means(number_of_centers, sdf_vector);
    std::vector<int> center_memberships;
    k_means.fill_with_center_ids(center_memberships);
    internal::Expectation_maximization gmm_k_means_init(number_of_centers, sdf_vector, center_memberships);
...
template <class Polyhedron>
inline void Surface_mesh_segmentation<Polyhedron>::apply_graph_cut_multiple_run(int number_of_run)
{
    //assign an id for every facet (facet-id)
    std::map<Facet_handle, int> facet_indices;
    int index = 0;
    ...
    for(int i = 0; i < number_of_run; ++i)
    {
        ...
        //apply graph cut
        double new_result;
        internal::Alpha_expansion_graph_cut gc(edges, edge_weights, probability_matrix, labels, &new_result);
...
inline void Surface_mesh_segmentation<Polyhedron>::apply_graph_cut_with_EM()
{
    ...
    internal::Alpha_expansion_graph_cut_with_EM gc(edges, edge_weights, labels, sdf_vector, probability_matrix, center_ids);
...
template <class Polyhedron>
inline void Surface_mesh_segmentation<Polyhedron>::depth_first_traversal(const Facet_handle& facet, int segment_id)
{
...
template <class Polyhedron>
inline void Surface_mesh_segmentation<Polyhedron>::write_segment_ids(const char* file_name)
{
    assign_segments();
    std::ofstream output(file_name);
    for(Facet_iterator facet_it = mesh->facets_begin(); facet_it != mesh->facets_end(); ++facet_it)
...
template <class Polyhedron>
inline void Surface_mesh_segmentation<Polyhedron>::read_probabilities(const char* file_name, std::vector<std::vector<double> > & probability_matrix)
{
    std::ifstream input(file_name);
